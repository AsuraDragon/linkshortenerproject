...
export type CreateLinkResult = { success: true; link: Link } | { success: false; error: string };

export async function createLinkAction(prevState: CreateLinkResult | null, formData: FormData): Promise<CreateLinkResult> {
    ...
    if (finalCode) {
        ...
        const existing = await db.select({ id: links.id }).from(links).where(eq(links.code, finalCode)).limit(1);
        ...
        while (!isUnique && attempts < 5) {
            ...
            const existing = await db.select({ id: links.id }).from(links).where(eq(links.code, candidate)).limit(1);
